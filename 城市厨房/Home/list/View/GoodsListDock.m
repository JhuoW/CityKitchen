...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        // 排序字段
        _fieldArray = @[@"price", @"sale", @"view", @"time"];
        //价格排序
        _isPrice = 0;
        [self addBtn:@"价格" index:0];
        [self addBtn:@"销量" index:1];
        [self addBtn:@"人气" index:2];
        [self addBtn:@"时间" index:3];
        self.userInteractionEnabled = YES;
        
        _selectedView = [[UIView alloc]init];
        _selectedView.backgroundColor = [UIColor clearColor];
        _selectedView.frame = CGRectMake(-(self.frame.size.width / 4), self.frame.size.height - 2, self.frame.size.width / 4, 3);
...
- (UIButton *)addBtn:(NSString *)optionName index:(int) i
{
    ...
    // 设置图片
    if (i == 0) {
        [btn setImage:[UIImage imageNamed:@"pricetoclick.png"]forState:UIControlStateNormal];
//        if (_isPrice) {
...
//        
        ...
        [btn setImage:[UIImage imageNamed:@"toclick.png"]forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:@"clicked.png"] forState:UIControlStateSelected];
    }
...
- (void)btnClick:(UIButton *)btn
{
    ...
    if (btn.tag == 0) {
        ...
        if (_isPrice) {
            _sortPrice = @"desc";
            [btn setImage:[UIImage imageNamed:@"desc.png"] forState:UIControlStateSelected];
        }else{
            _sortPrice = @"asc";
            [btn setImage:[UIImage imageNamed:@"asc.png"] forState:UIControlStateSelected];
        }
        if ([_delegate respondsToSelector:@selector(goodsListDockField:SortedBy:)]) {
            [_delegate goodsListDockField:_fieldArray[btn.tag] SortedBy:_sortPrice];
        }
    }else{
        if ([_delegate respondsToSelector:@selector(goodsListDockField:SortedBy:)]) {
            [_delegate goodsListDockField:_fieldArray[btn.tag] SortedBy:@"desc"];
        }
    }
    btn.selected = YES;
